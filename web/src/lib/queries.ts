...
export async function findAllProjects(api: Api): Promise<Array<Project>> {
  try {
    const data = await api.request<ProjectsData>(
      `
      query Projects {
        projects {
          nodes {
...
async function findIssuesRelatedToProject(
  ...
  do {
    ...
    console.log(`Doing issue query page ${page} ${afterText} ...`)
...
async function findRelatedIssuesPaginated(
  ...
  try {
    const data = await api.request<DependenciesData>(
      `
